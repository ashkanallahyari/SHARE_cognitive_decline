...
##### Training LongituRF model (MERT)
# =====================================================
# Running MERT model with updated unique ID
smert_model <- LongituRF::MERT(
  X = data.frame(df_list_train$X),  # Convert predictors to data frame
  Y = df_list_train$Y,  # Outcome variable
  id = df_list_train$id,  # Subject ID (now properly formatted)
  Z = df_list_train$Z,  # Random effects
...
model_filename_rds <- paste0("models/smert_model_", target_value_name, ".rds")
model_filename_rdata <- paste0("models/smert_model_", target_value_name, ".RData")

# Save the model with dynamic names
saveRDS(smert_model, file = model_filename_rds)
save(smert_model, file = model_filename_rdata)


# Extract the tree
tree <- smert_model$forest
...
jpeg("figures/MERT_decision_tree.jpg", width = 800, height = 300)

rpart.plot(tree, 
           main = "MERT Decision Tree",
...
# Save the tree plot as an image
jpeg("figures/MERT_decision_treefull.jpg", width = 1600, height = 800)
...
##### Extract variable importance from MERT model
# =====================================================
variable_importance_smert <- tibble(
  variables = names(smert_model$forest$variable.importance),  
  importance = smert_model$forest$variable.importance
)


# Create importance plot
importance_plot_MERT <- ggplot(variable_importance_smert) +
  geom_col(aes(y = fct_reorder(variables, importance), x = importance), show.legend = FALSE) +
  labs(y = element_blank(), x = "Importance") +
  ggtitle("MERT Variable Importance") +
...
ggsave("figures/importance_plot_MERT.jpg", plot = importance_plot_MERT, width = 8, height = 6, dpi = 300)
...
# Making predictions for MERT on train dataset
pred_train <- predict(
  smert_model,
...
train_mae_MERT <- mae(df_train$cf_score, df_train$pred_train)

# Calculate RMSE for both models
train_rmse_MERT <- rmse(df_train$cf_score, df_train$pred_train)

# Calculate R² for both models
train_r2_MERT <- cor(df_train$cf_score, df_train$pred_train)^2
...
# Making predictions for MERT on test dataset
df_list_test$Z <- matrix(rep(1, nrow(df_list_test$Z)), ncol = 1)
# df_list_test$X <- as.data.frame(df_list_test$X)
# df_list_test$Z <- as.data.frame(df_list_test$Z)
...
test_mae_MERT <- mae(df_test$cf_score, df_test$pred_test)

# Calculate RMSE for both models
test_rmse_MERT <- rmse(df_test$cf_score, df_test$pred_test)

# Calculate R² for both models
test_r2_MERT <- cor(df_test$cf_score, df_test$pred_test)^2


# Print results
print(glue::glue("Train MAE MERT: {train_mae_MERT}   |   Test MAE MERT: {test_mae_MERT}"))
print(glue::glue("Train RMSE MERT: {train_rmse_MERT}   |   Test RMSE MERT: {test_rmse_MERT}"))
print(glue::glue("Train R² MERT: {train_r2_MERT}   |   Test R² MERT: {test_r2_MERT}"))
...
report_file <- "reports/MERT_model_performance.txt"

# Prepare performance results
performance_results <- glue::glue("
Train MAE MERT: {train_mae_MERT}   |   Test MAE MERT: {test_mae_MERT}
Train RMSE MERT: {train_rmse_MERT}   |   Test RMSE MERT: {test_rmse_MERT}
Train R² MERT: {train_r2_MERT}   |   Test R² MERT: {test_r2_MERT}
")
...
train_filename <- paste0("data/MERT_df_train_", target_value_name, ".rds")
test_filename <- paste0("data/MERT_df_test_", target_value_name, ".rds")
